...
// Re-fetches when clienteId changes. Uses AbortController to cancel in-flight
// requests when clienteId changes or the component unmounts.
export function useFichaCliente(clienteId: number): UseFichaClienteReturn {
  ...
  useEffect(() => {
    ...
    setIsLoading(true);
    setError(null);
    obtenerFichaCliente(port, clienteId, ctrl.signal)
      .then((out) => {
